...
def parse_kerberos_output(stdout: str, target: str):
    nodes, edges = [], []
    for line in stdout.splitlines():
        ls = line.strip()
        if "AS-REP roastable" in ls:
            user = ls.split()[-1]
            nodes.append({"id": f"USER:{user}", "type": "User", "attrs": {"asrep_roastable": True}})
        if "ServicePrincipalName" in ls:
            parts = ls.split()
            if len(parts) >= 2:
                owner = parts[0]
                spn = parts[-1]
                nodes.append({"id": f"SPN:{spn}", "type": "SPN", "attrs": {}})
                edges.append({"src": f"USER:{owner}", "dst": f"SPN:{spn}", "type": "HasSPN"})
    return nodes, edges

class KerberosEnum(Module):
    name = "kerberos_enum"
    supported_auth = {"password", "ntlm", "aes", "ticket", "cert", "null"}

    async def run(self, session, target):
        start = time.strftime("%Y-%m-%dT%H:%M:%S")
        out_dir = f"out/{target.host}/kerberos"
        os.makedirs(out_dir, exist_ok=True)

        cmd = ["nxc", "kerberos", target.host]
        if session.domain:
            cmd += ["-d", session.domain]
        ...
        nodes, edges = parse_kerberos_output(so, target.host)
